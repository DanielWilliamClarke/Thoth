...
>Logs generated by the `Thoth` service when running with the stack after funneled to a log file for comsumption
when running standalone via `npm run start` logs are dumped to the console.

## Postman

- You can import the provided postman collection and environment into postman to faciliate generating traffic against the Thoth service
